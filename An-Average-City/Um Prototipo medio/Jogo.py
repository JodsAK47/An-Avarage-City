...
class GerenciadorJogo:
    ...
    def _aplicar_classe_personagem2(self):
        if getattr(self, 'personagem2', None) is None:
            return
        verso = getattr(self, 'classe_jogador2', None)
        classe_dano = getattr(self, 'classe_jogador2_tipo', None)
        if not verso or not classe_dano:
            return

        chave_aplicada = f"{verso}-{classe_dano}"
        if getattr(self.personagem2, 'classe_aplicada', None) == chave_aplicada:
            return

        ataques = [nome for nome, hab in banco_habilidades.items() if hab.verso == verso and hab.tipo == classe_dano]
        self.personagem2.ataques = ataques

        if classe_dano == "Físico":
            self.personagem2.atributos["For"] = int(self.personagem2.atributos.get("For", 1)) + 3
        elif classe_dano == "Magia":
            self.personagem2.atributos["Int"] = int(self.personagem2.atributos.get("Int", 1)) + 3
        elif classe_dano == "Distância":
            self.personagem2.atributos["Agi"] = int(self.personagem2.atributos.get("Agi", 1)) + 3

        self.personagem2.is_player = True
        self.personagem2.classe_aplicada = chave_aplicada
    ...
    def configurar_classe(self, novo_verso, nova_classe_dano):
        if getattr(self, "classe_dano_atual", None) and getattr(self, "classe_dano_atual") != nova_classe_dano:
            velha_classe = self.classe_dano_atual
            if velha_classe == "Físico":
                personagem.atributos["For"] = max(1, personagem.atributos.get("For", 1) - 3)
            elif velha_classe == "Magia":
                personagem.atributos["Int"] = max(1, personagem.atributos.get("Int", 1) - 3)
            elif velha_classe == "Distância":
                personagem.atributos["Agi"] = max(1, personagem.atributos.get("Agi", 1) - 3)

        if self.classe_atual != novo_verso or getattr(self, "classe_dano_atual", None) != nova_classe_dano:
            self.classe_atual = novo_verso
            self.classe_dano_atual = nova_classe_dano
            
            ataques = [nome for nome, hab in banco_habilidades.items() if hab.verso == novo_verso and hab.tipo == nova_classe_dano]
            personagem.ataques = ataques

            if nova_classe_dano == "Físico":
                personagem.atributos["For"] = personagem.atributos.get("For", 1) + 3
            elif nova_classe_dano == "Magia":
                personagem.atributos["Int"] = personagem.atributos.get("Int", 1) + 3
            elif nova_classe_dano == "Distância":
                personagem.atributos["Agi"] = personagem.atributos.get("Agi", 1) + 3
                
            from Inventario import inventario_global, Equipamento
            eq_nome = "Equipamento Padrão"
            eq_attr = {}
            if novo_verso == "Sobrevivente":
                if nova_classe_dano == "Físico": eq_nome, eq_attr = "Soco Inglês", {"For": 2}
                elif nova_classe_dano == "Distância": eq_nome, eq_attr = "Estilingue", {"Agi": 2}
                elif nova_classe_dano == "Magia": eq_nome, eq_attr = "Amuleto Rústico", {"Int": 2}
            elif novo_verso == "Agente":
                if nova_classe_dano == "Físico": eq_nome, eq_attr = "Faca de Combate", {"For": 2}
                elif nova_classe_dano == "Distância": eq_nome, eq_attr = "Pistola Silenciada", {"Agi": 2}
                elif nova_classe_dano == "Magia": eq_nome, eq_attr = "Relógio Arcâno", {"Int": 2}
            elif novo_verso == "Feiticeiro":
                if nova_classe_dano == "Físico": eq_nome, eq_attr = "Cajado de Batalha", {"For": 2}
                elif nova_classe_dano == "Distância": eq_nome, eq_attr = "Varinha Mágica", {"Agi": 2}
                elif nova_classe_dano == "Magia": eq_nome, eq_attr = "Grimório Sombrio", {"Int": 2}
            
            inventario_global.equipar(Equipamento(eq_nome, eq_attr))
    ...
    def configurar_cenario(self, id_cenario):
        ...
        if self.partida_atual >= self.total_partidas:
            ...
            chefe = Inimigo(0, 0, 500, "Sukuna1.png", "O Grande Mal", ["Porrete do Chefe", "Fogo", "Nevasca", "Terremoto"], 1000, {"For": 10, "Agi": 6, "Int": 10})
            inimigos_fase = [chefe]
        ...
        for ent in self.ordem_turnos:
            if not getattr(ent, 'is_player', False):
                ent.hp = ent.max_hp
    ...
    def reiniciar(self):
        ...
        for entidade in self.ordem_turnos:
            ...
            if not getattr(entidade, 'is_player', False):
    ...
    def recuperar_apos_batalha(self, dormir=False):
        percentual_hp = 0.60 if dormir else 0.30
        pontos_mp = 6 if dormir else 3

        for jogador in self.obter_jogadores():
            jogador.hp = min(jogador.max_hp, jogador.hp + int(jogador.max_hp * percentual_hp))
            jogador.mp = min(jogador.max_mp, jogador.mp + pontos_mp)
    ...
    def calcular_bonus_atributo(self, atacante, tipo_ataque):
        ...
        bonus = 0
        if tipo_ataque == "Físico": bonus = atributos.get("For", 0)
        elif tipo_ataque == "Distância": bonus = atributos.get("Agi", 0)
        elif tipo_ataque == "Magia": bonus = atributos.get("Int", 0)
        
        if getattr(atacante, 'is_player', False):
            from Inventario import inventario_global
            eq = inventario_global.equipamento_slot
            if eq:
                if tipo_ataque == "Físico": bonus += eq.atributos_bonus.get("For", 0)
                elif tipo_ataque == "Distância": bonus += eq.atributos_bonus.get("Agi", 0)
                elif tipo_ataque == "Magia": bonus += eq.atributos_bonus.get("Int", 0)
        return bonus
    ...
    def atualizar(self, posicao_mouse, estado_clique_mouse):
        ...
        if not inimigos_vivos:
            if not self.xp_calculado:
                ...
                from Inventario import inventario_global
                qnt_inimigos = len([e for e in self.ordem_turnos if not getattr(e, 'is_player', False)])
                drop_msg = []
                for _ in range(qnt_inimigos):
                    if random.random() < 0.3:
                        inventario_global.adicionar_consumivel("Poção de Vida", "Restaura 30 HP", 30, 0)
                        drop_msg.append("Vida")
                    elif random.random() < 0.2:
                        inventario_global.adicionar_consumivel("Poção de Mana", "Restaura 10 MP", 0, 10)
                        drop_msg.append("Mana")
                    if random.random() < 0.4:
                        inventario_global.adicionar_material("Sucata", "Melhora equips.")
                        drop_msg.append("Sucata")
                
                if drop_msg:
                    self.mensagem_log = f"🎁 Drops: {', '.join(drop_msg)}"
        ...
        if self.menu_acoes_estado == "esquivando":
            ...
            if entidade_atual.congelado:
                ...
                if self.timer_inimigo >= self.tempo_espera_inimigo:
                    ...
                    hab_usada = entidade_atual.obter_habilidade(nome_golpe_inimigo)
