...
class GerenciadorJogo:
    def __init__(self, tela):
        ...
        # BOTÕES DA HUD (Realinhados para caber no box e ajustados no eixo Y)
    ...
    def gerar_botoes_ataque(self):
        ...
        for i, nome_hab in enumerate(personagem.ataques):
            x = 220 + (i * 140) # Ajustado para acompanhar o novo alinhamento
            ...
            if personagem.congelado:
                ...
                if self.menu_acoes_estado == "principal":
                    if "usou" not in self.mensagem_log and "ERROU" not in self.mensagem_log and "falhou" not in self.mensagem_log.lower() and "CRÍTICO" not in self.mensagem_log:
                        self.mensagem_log = "Seu Turno!" # <-- TEXTO DO LOG LIMPO
    ...
    def desenhar(self):
        ...
        # 1. LOG DE BATALHA MOVIDO PARA O TOPO (Y = 20)
        ...
        # 2. BARRA DE AÇÕES (Movida para Y = 600 para dar espaço para o painel de atributos abaixo)
        ...
        # 3. NOVO PAINEL DE ATRIBUTOS (Fica logo abaixo da barra de ações em Y = 720)
        ...
        # Desenhar barras de Vida (HP) e Mana (PM) do jogador dentro do hud_status com números
